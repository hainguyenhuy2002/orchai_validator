...
def dec_score(df:pd.DataFrame, col:str):
   ...
   def calculate(df: pd.DataFrame):
       ...
       df['ln(pi)'] = df['pi'].apply(lambda x: 0 if (x ==0) else log(x))
...
def cal_reward(row: pd.Series, col_in, col_out):
   row[col_out] = row[col_in] * (1.0 - row["commission_rate"])
   return row
...
if __name__ == "__main__":
   ...
   dt = back_test_reward(df, start=7059473, end=9151673, hop_size=14400, win_size=432000, col="score")
  
   print(dt[dt["real_reward"] <= dt["fake_reward"]].shape[0] / dt.shape[0])


   summarize, df_decentralized = back_test_decentralized(df, start=7059473, end=9151673, hop_size=14400, win_size=432000, predicted_col="score") 
